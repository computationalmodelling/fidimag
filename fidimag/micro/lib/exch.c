...
void compute_exch_field_aniso_micro(double *restrict m, double *restrict field,
			                        ...
			                        double *restrict Ms_inv, double gam,
                                    double dx, double dy, double dz,
                                    int n, int *restrict ngbs) {

    /* Compute the micromagnetic anisotropic exchange field and energy using the
     * matrix of neighbouring spins and a second order approximation
     * for the derivative. Same construction as the exchange term
     * AUTHOR: Sam Moody. Durham University. 1 / 11 / 2021
     *
     * Ms_inv     :: Array with the (1 / Ms) values for every mesh node.
     *               The values are zero for points with Ms = 0 (no material)
     *
     ...
     *              since it is the left neighbour which is the PBC in x, etc..)
     *
     *  For the ANISOTROPIC exchange computation, the field is defined as:
     *          H_ex = (2 * gam / (mu0 * Ms)) * \sum_{i=x,y,z} (d^2(m_i)/di^2) 
     *  which are only 3 of the terms from typical exchange interaction
     *
     *  Therefore, for the i-th mesh node (spin), we approximate the
     *  derivatives as:
     *          d^2 mx/dx^2 = (1 / dx^2) * ( m[i-x] - 2 * m[i] + m[i+x] )
     *
     *  Where i-x is the neighbour in the -x direction. This is similar
     *  for my and mz.
     *  We can notice that the sum is the same if we do:
    ...
    /* Define the coefficients */
	double ax = 2 * gam / (dx * dx);
    double ay = 2 * gam / (dy * dy);
    double az = 2 * gam / (dz * dz);
